...
std::unique_ptr<parquet::arrow::FileReader> openFile(std::string filePath){
    ...
    std::shared_ptr<arrow::io::RandomAccessFile> input = arrow::io::ReadableFile::Open(filePath).ValueOrDie();
...
daxl::Status createNewTable(DiskTable *table, std::unique_ptr<parquet::arrow::FileReader> *reader, std::list<std::tuple<std::string, std::shared_ptr<arrow::DataType>, std::string>> const &partitions){
    ...
    for (uint i = 0; i < schema->num_fields(); i++){
        ...
        table->addColumn(Column(type, precision, scale, name, is_dictionary_encoded));
    }
...
std::shared_ptr<arrow::Array> createAdditionalColumnData(std::shared_ptr<arrow::DataType> datatype, std::string strValue, int rowNumber){
...
void insert_data_into_table(std::string tableName, std::unique_ptr<parquet::arrow::FileReader> *reader,
                            DiskTable *table,
                            int batch_size,
                            Session *session,
                            std::list<std::tuple<std::string,
                                                 std::shared_ptr<arrow::DataType>, std::string>> const &partitions)
{
        ...
        while (hasNextBatch){
            ...
            if (recB != NULL && recB.get() != NULL){
                ...
                for (auto &tup : partitions){
                    ...
                    recB = recB->AddColumn(colum_number + j, fieldName, array).ValueOrDie();
                    j++;
                }
...
int importSingleFile(std::filesystem::path filePath, int batch_size, Session *session, std::list<std::tuple<std::string, std::shared_ptr<arrow::DataType>, std::string>> const &partitions, std::string tableName = ""){
    ...
    if (status.isOk()){
        insert_data_into_table(tableName, &reader, &table, batch_size, session, partitions);
    }
    else{
...
std::string createPartitionValues(std::filesystem::path filePath,
                                  std::list<std::tuple<std::string, std::shared_ptr<arrow::DataType>>> const &partitions,
                                  std::list<std::tuple<std::string, std::shared_ptr<arrow::DataType>, std::string>> &partitionsWithValues){
    ...
    for (auto it = partitions.begin(); it != partitions.end() && !stackPartitionValues.empty(); ++it){
...
void importSingleFileWithPartitions(std::filesystem::path filePath,  int batch_size, Session *session, std::list<std::tuple<std::string, std::shared_ptr<arrow::DataType>>> const &partitions){
        std::list<std::tuple<std::string, std::shared_ptr<arrow::DataType>, std::string>> partitionsWithValue;
        std::string table_name = filePath.filename();
        if (!partitions.empty()){
            table_name = createPartitionValues(filePath, partitions, partitionsWithValue);
        }
        importSingleFile(filePath, batch_size, session, partitionsWithValue, table_name);
 }

void importDirectory(std::filesystem::path const &filePath, int batch_size, Session *session, std::list<std::tuple<std::string, std::shared_ptr<arrow::DataType>>> partitions){
     for (auto const &entry : std::filesystem::recursive_directory_iterator(filePath)){
            if (std::filesystem::is_regular_file(entry) && entry.path().extension() == ".parquet"){
                importSingleFileWithPartitions(entry.path(), batch_size, session, partitions);
            }
        }
}


void importByPath(std::filesystem::path const &filePath, int batch_size, std::list<std::tuple<std::string, std::shared_ptr<arrow::DataType>>> partitions){
...
std::tuple<std::string, std::shared_ptr<arrow::DataType>> parsePartition(std::string strPartition){
...
std::list<std::tuple<std::string, std::shared_ptr<arrow::DataType>>> parsePartitionsParameter(std::string strPartitions){
    std::string delimiter = "/";
    size_t pos = 0;
...
int main(int argc, char *argv[]){
    ...
    std::list<std::tuple<std::string, std::shared_ptr<arrow::DataType>>> partitions;
